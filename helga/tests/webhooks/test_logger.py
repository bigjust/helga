...
class TestIndexView:
    ...
    def test_channels_hides_blacklist(self, monkeypatch):
        ...
        monkeypatch.setattr(logger, "settings", Mock(CHANNEL_LOGGING_HIDE_CHANNELS=["#foo", "bar"]))
...
class TestChannelLogView:
    ...
    def test_messages_with_multiline_content(self, tmpdir):
        ...
        file.write(
            "\n".join(
                ["00:00:00 - foo - this is what i said", "...and here", "...and here again", "...☃"]
            ),
            mode="w",
        )
    ...
    def test_download(self, tmpdir):
        ...
        request.setHeader.assert_any_call(
            "Content-Disposition", "attachment; filename=2014-12-01.txt"
        )
...
class TestWebhook:
    ...
    def test_renders_channel_log_as_text(self, tmpdir):
        ...
        self.request.setHeader.assert_any_call(
            "Content-Disposition", "attachment; filename=2014-12-01.txt"
        )
