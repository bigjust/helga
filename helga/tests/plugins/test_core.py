...
class TestRegistry:
    ...
    def test_prioritized(self):
        ...
        registry.enabled_plugins["#bots"] = {"foo", "bar"}
    ...
    def test_process_returns_all_responses(self):
        ...
        with patch.object(registry, "prioritized") as prio:
            ...
            assert response == ["foo", "bar", self.snowman, "baz"]
    ...
    def test_process_returns_unicode(self):
        ...
        with patch.object(registry, "prioritized") as prio:
            ...
            assert all(isinstance(x, str) for x in responses)
    ...
    def test_register_plugin_handles_unicode(self):
        ...
        for plugin in self.valid_plugins:
            name = f"{self.snowman}-{repr(plugin)}"
    ...
    def test_all_plugins(self):
        ...
        assert {"foo"} == registry.all_plugins
    ...
    def test_load(self, signal, iter_entry_points, logger):
        ...
        with patch.multiple(
            registry,
            register=Mock(),
            whitelist_plugins={"foo", self.snowman, "bar"},
...
class TestPlugin:
    ...
    def test_preprocess_with_priority(self):
        ...
        assert expected == foo(*args)
        assert expected == foo._plugins[0].preprocess(*args)
        assert foo._plugins[0].priority == 10
...
class TestCommand:
    ...
    def test_parse_handles_aliases(self):
        assert self.cmd.parse("helga", "helga bar")[0] == "bar"
        assert self.cmd.parse("helga", "helga baz")[0] == "baz"
    ...
    def test_parse_returns_args(self):
        assert self.cmd.parse("helga", "helga foo 1 2 3")[1] == ["1", "2", "3"]

    def test_parse_handles_longest_command_first(self):
        with patch.object(self.cmd, "aliases", ["b", "bar"]):
            for check in ("b", "bar"):
                cmd, args = self.cmd.parse("helga", f"helga {check} baz")
                assert cmd == check
                assert args == ["baz"]
    ...
    def test_parse_handles_unicode(self):
        snowman = "☃"
        disapproval = "ಠ_ಠ"
        cmd = f"helga {snowman} {disapproval}"
    ...
    def test_process_calls_class_run_method(self):
        ...
        assert self.cmd.process(self.client, "#bots", "me", "helga foo") == "run"
    ...
    def test_process_calls_custom_runner(self):
        ...
        assert cmd.process(self.client, "#bots", "me", "helga foo") == "runner"
    ...
    def test_multiple_decorators(self):
        ...
        assert foobar._plugins[0](self.client, "#bots", "me", "helga bar") == "bar"
        assert foobar._plugins[1](self.client, "#bots", "me", "helga foo") == "foo"
    ...
    def test_decorator_using_command(self):
        ...
        assert foo._plugins[0](self.client, "#bots", "me", "helga foo") == "bar"
    ...
    def test_decorator_using_alias(self):
        ...
        assert foo._plugins[0](self.client, "#bots", "me", "helga baz") == "bar"
...
class TestMatch:
    ...
    def test_match_using_callable(self):
        ...
        assert self.match.match("this is a foo message") == "foobar"

    def test_match_using_simple_pattern(self):
        self.match.pattern = r"foo-(\d+)"
        assert self.match.match("this is about foo-123") == ["123"]
    ...
    def test_simple_decorator(self):
        ...
        assert foo._plugins[0](self.client, "#bots", "me", "this is about foo-123") == "123"
    ...
    def test_callable_decorator(self):
        ...
        assert foo._plugins[0](self.client, "#bots", "me", "foo at the start") == "bar"
    ...
    def test_match_with_unicode(self):
        ...
        assert snowman_match._plugins[0](self.client, "#bots", "me", "☃") == "snowman"
