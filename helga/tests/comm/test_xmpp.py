...
class ClientTestCase(TestCase):
    ...
    def test_ping(self, uuid):
        ...
        with patch.object(self.client, "stream"):
            ...
            assert (
                '<ping xmlns="urn:xmpp:ping"' in output or "<ping xmlns='urn:xmpp:ping'" in output
            )
    ...
    def test_get_channel_logger_no_existing_logger(self, log):
        ...
        assert self.client.get_channel_logger("#foo") == "foo"
    ...
    def test_get_channel_logger_existing_logger(self, log):
        ...
        assert self.client.get_channel_logger("#foo") == "bar"
    ...
    def test_on_invite_direct(self):
        ...
        x = xmpp.domish.Element(
            ("jabber:x:conference", "x"), attribs={"jid": "room@conf.example.com"}
        )
        element.addChild(x)
    ...
    def test_on_invite_ignores_probably_not_invite(self):
        element = xmpp.domish.Element(("", "message"))
        x = xmpp.domish.Element(("", "x"), attribs={"jid": "room@conf.example.com"})
        element.addChild(x)
    ...
    def test_join(self):
        expected = xmpp.domish.Element(
            ("jabber:client", "presence"),
            attribs={
                "to": f"bots@conf.example.com/{self.client.nickname}",
                "from": self.client.jid.full(),
            },
        )
        muc = xmpp.domish.Element(("http://jabber.org/protocol/muc", "x"))
        hist = xmpp.domish.Element(
            ("", "history"),
            attribs={
                "maxchars": "0",
                "maxstanzas": "0",
    ...
    def test_join_with_simple_room_name(self):
        expected = xmpp.domish.Element(
            ("jabber:client", "presence"),
            attribs={
                "to": f"bots@{self.client.conference_host}/{self.client.nickname}",
                "from": self.client.jid.full(),
            },
        )
        muc = xmpp.domish.Element(("http://jabber.org/protocol/muc", "x"))
    ...
    def test_leave_with_simple_room_name(self):
        expected = xmpp.domish.Element(
            ("jabber:client", "presence"),
            attribs={
                "to": f"bots@{self.client.conference_host}",
                "from": self.client.jid.full(),
                "type": "unavailable",
            },
        )
    ...
    def test_format_channel_with_simple_chan(self):
        expected = f"bots@{self.client.conference_host}"
